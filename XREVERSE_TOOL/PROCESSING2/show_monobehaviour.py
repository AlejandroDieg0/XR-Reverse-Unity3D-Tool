...
patternStr = r"(MonoBehaviour)_((\d+)_ID(\d+))"  #Objects da filtrare
pattern2Str = r"m_Script" #attributo da salvare
pattern3Str = r"m_GameObject" #attributo da salvare
...
def render(ast, name=None):
    ...
    if type(ast) is dict: #renderizzo la root
        for key,val in ast.iteritems():
            ...
            if containsPattern: #grafico solo MonoBehaviour (generico)
                if type(val) is dict:
                    child = getID()
                    edges.append((name, child, asAttr("label", ""))) #archi che vanno da root a monobehaviour
    ...
    if type(ast) is dict:
        ...
        for key,val in ast.iteritems():
            ...
            if type(val) is dict: #se hanno file collegati come attributi (fileid=0)
                ...
                contains3Pattern = re.search(pattern3Str, key)
                if contains2Pattern:
                    lab += (str(key) + ": " + str(val) + "\\n")
                if contains3Pattern:
                    edges.append((name, child, asAttr("label", ""))) #archi che vanno da root a monobehaviour
                    render3(val, key, child) #vado ad analizzare i figli dei gameobject, passo il nome del nodo come param
...
def render3(ast, nodo, name=None): #rendering dei componenti
    if name is None:
        name = getID()
