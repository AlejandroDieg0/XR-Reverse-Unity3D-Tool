...
patternStr = r""  #Objects da filtrare
pattern2Str = r"" #attributo da salvare
...
def render(ast, name=None):
    ...
    if type(ast) is dict: #renderizzo la root
        for key,val in ast.iteritems():
            ...
            if type(val) is dict:
                child = getID()
                edges.append((name, child, asAttr("label", key))) #archi che vanno da root a gameobject
                render2(val, child) #vado ad analizzare i figli dei gameobject
...
def render2(ast, name=None): #renderizzo i gameobjects
    if name is None:
        name = getID()
